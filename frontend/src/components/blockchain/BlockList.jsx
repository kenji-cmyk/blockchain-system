...
function BlockList({ blocks, compact = false }) {
  ...
  return (
    <div className={`custom-scrollbar grid gap-3 overflow-auto pr-1 ${compact ? "max-h-[410px]" : "max-h-[calc(100vh-220px)]"}`}>
      {blocks.map((block) => (
        <a key={`${block.index}-${block.hash}`} href={detailHash("blocks", block.index)} className="grid gap-3 rounded-xl border border-white/10 bg-void/45 p-4 transition hover:border-lime/40 md:grid-cols-[88px_minmax(0,1fr)_auto]">
